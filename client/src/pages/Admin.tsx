...
function trackLabel(tx: Tx, track: AdminUser["track"]): string {
  if (track === "medecine") return tx("Médecine", "Medicine");
  if (track === "dentaire") return tx("Dentaire", "Dentistry");
  return "—";
}

function trialCountdown(trialEndsAt: number, tx: Tx): string {
  const remaining = Math.max(0, trialEndsAt - Date.now());
  if (remaining === 0) return tx("terminé", "ended");
  const hours = Math.floor(remaining / 3_600_000);
  const minutes = Math.floor((remaining % 3_600_000) / 60_000);
  return `${hours}:${String(minutes).padStart(2, "0")} ${tx("restantes", "remaining")}`;
}

function statusLabel(tx: Tx, user: AdminUser): string {
  if (user.role === "admin") return tx("Administrateur", "Administrator");
  if (user.subscriptionStatus === "active") return tx("Accès permanent", "Permanent access");
  if (user.subscriptionStatus === "trialing" && user.trialEndsAt) {
    return `${tx("Essai gratuit actif", "Free trial active")} — ${trialCountdown(user.trialEndsAt, tx)}`;
  }
  if (user.trialStatus === "expired") return tx("Essai gratuit terminé", "Free trial ended");
  return tx("Aucun accès", "No access");
}
...
export function Admin() {
  ...
  return (
    <main className="admin-shell">
      <Link to="/library" className="back-link">{tx("← Retour à la bibliothèque", "← Back to library")}</Link>
      <header><p className="eyebrow">{tx("Espace Jessica", "Jessica workspace")}</p><h1>{tx("Administration de première année", "Year-one administration")}</h1><p>{tx("Ouvre ou ferme un semestre pour les étudiants ayant un accès actif.", "Open or close a semester for students with active access.")}</p></header>
        ...
        <section className="admin-semester-list">
          {semesters.map((semester) => (
            <article key={semester.id} className="admin-semester-row">
              <div><span className={`status-dot ${semester.is_published ? "is-open" : ""}`} /><p className="eyebrow">{tx("Première année", "Year one")}</p><h2>{lang === "fr" ? semester.title_fr : semester.title_en}</h2><p>{lang === "fr" ? semester.description_fr : semester.description_en}</p></div>
              <div className="admin-actions"><strong>{semester.is_published ? (tx("Accessible", "Open")) : (tx("Fermé", "Closed"))}</strong><button type="button" onClick={() => void toggle(semester)} disabled={saving === semester.semester_number}>{saving === semester.semester_number ? "…" : semester.is_published ? (tx("Fermer le semestre", "Close semester")) : (tx("Ouvrir le semestre", "Open semester"))}</button></div>
            </article>
          ))}
        </section>
      )}
      ...
      <header className="admin-users-header">
        <h1>{tx("Comptes étudiants", "Student accounts")}</h1>
        <p>
          {tx("L'essai gratuit de 48h est accordé automatiquement à l'inscription. Depuis ici, tu peux aussi réaccorder 48h à un compte (ex : après un bug du site), couper son accès, ou lui ouvrir un chapitre en particulier.", "The 48h free trial is granted automatically at signup. From here you can also re-grant 48h to an account (e.g. after a site outage), cut its access, or open a specific chapter for it.")}
        </p>
      </header>
      {!users ? (
            ...
            return (
              <li key={u.id} className="admin-student-row">
                <button type="button" className="admin-student-toggle" onClick={() => void toggleUser(u.id)}>
                  ...
                  <span className="admin-status-badge">{statusLabel(tx, u)}</span>
                </button>
                <div className="admin-student-meta">
                  <span className="admin-meta-chip">{trackLabel(tx, u.track)}</span>
                  {(u.phoneCountryCode || u.phoneNumber) && (
                    <span className="admin-meta-chip">{u.phoneCountryCode ?? ""} {u.phoneNumber ?? ""}</span>
                  )}
                  <span className="admin-meta-chip">{tx("Inscrit le ", "Registered ")}{formatDate(locale, u.createdAt)}</span>
                </div>
                <div className="admin-student-actions">
                  {u.role !== "admin" && (
                    <button type="button" onClick={() => void grantTrial(u.id)} disabled={accessSaving === u.id}>
                      {accessSaving === u.id ? "…" : tx("Accorder 48h", "Grant 48h")}
                    </button>
                  )}
                  {u.role !== "admin" && u.subscriptionStatus !== "inactive" && (
                    <button type="button" className="secondary" onClick={() => void revokeAccess(u.id)} disabled={accessSaving === u.id}>
                      {accessSaving === u.id ? "…" : tx("Couper l'accès", "Cut access")}
                    </button>
                  )}
                </div>
                {expandedUserId === u.id && (
